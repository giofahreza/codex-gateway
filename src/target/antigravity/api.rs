...
fn build_google_contents(request_value: &Value) -> Result<Vec<Value>, String> {
    ...
    if let Some(items) = request_value.get("input").and_then(|v| v.as_array()) {
        ...
        for item in items {
            ...
            match item_type {
                ...
                "message" | "" => {
                    ...
                    if role == "user" {
                        // Keep the full parts list (text + images) so
                        // multimodal user turns are preserved. We also
                        // remember the text portion so the transcript
                        // prefix can be prepended to it later.
        ...
        if let Some(text) = last_user_text {
            ...
            // Find the existing user turn and prepend the transcript
            // to its first text part, so any non-text parts (images,
            // etc.) are preserved.
            let mut found = false;
