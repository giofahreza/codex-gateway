...
const GEMINI_QUOTA_ENDPOINTS: &[&str] = &[
    ...
    for account in &accounts {
        ...
        let entry = if let Some(cached) = cached {
            if now.duration_since(cached.fetched_at).as_secs() < 60 {
                ...
                let mut cache = state.gemini_quota_cache.lock().unwrap();
        ...
        if let Some(err) = entry.error {
            results.push(json!({
                "label": account.label,
                "email": account.email,
                "project_id": super::auth::request_project_id(account),
...
fn cache_key(account: &super::accounts::GeminiAccount) -> String {
    account
        .file_name
        .clone()
...
async fn fetch_account_quota(
    ...
    account: &super::accounts::GeminiAccount,
) -> QuotaCacheEntry {
    ...
    let tier_info = fetch_tier_info(&state.client, &access_token, &project_id).await;
    let quota_info = fetch_quota_summary(&state.client, &access_token, &project_id).await;
...
async fn fetch_tier_info(
    ...
    let current = value
        ...
        .ok_or_else(|| "loadCodeAssist missing tier information".to_string())?;
...
async fn fetch_quota_summary(
    ...
    if let Ok(value) = post_gemini_json(
        client,
        access_token,
    ...
    .await
    {
        if let Some(groups_value) = value.get("groups").and_then(|value| value.as_array()) {
    ...
    let model_names = fetch_model_display_names(client, access_token, &body)
        .await
        .unwrap_or_default();
    let models = fetch_model_quotas(client, access_token, &body, &groups, &model_names).await?;
...
async fn fetch_model_quotas(
    ...
    let value =
        post_gemini_json(client, access_token, "v1internal:retrieveUserQuota", body).await?;
    let buckets = value
        .get("buckets")
        .and_then(|value| value.as_array())
    ...
    for bucket in buckets {
        let Some(model_id) = bucket.get("modelId").and_then(|value| value.as_str()) else {
            continue;
        };
        if !is_user_facing_model(model_id) {
            continue;
        ...
        models.push(ModelQuotaSummary {
            ...
            five_hour: group.and_then(|group| group.five_hour.clone()),
            weekly: group.and_then(|group| group.weekly.clone()),
        });
    }
...
async fn post_gemini_json(
    ...
    let mut last_error = format!("all Gemini endpoints failed for {}", path);

    for endpoint in GEMINI_QUOTA_ENDPOINTS {
        ...
        let Ok(resp) = resp else {
            last_error = format!("failed to reach Gemini endpoint {}", path);
            continue;
        };
...
pub fn prune_cache(
    ...
    accounts: &[super::accounts::GeminiAccount],
) {
